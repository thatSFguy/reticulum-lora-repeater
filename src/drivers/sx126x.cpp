...
//
// Ported to reticulum-lora-repeater from microReticulum_Faketec_Repeater
// with targeted edits: replaced legacy Boards.h include with the
// sx126x_platform.h compatibility shim, and replaced per-board #if
// chains in enableTCXO() and preInit() with macro-driven arms sourced
// from include/board/<name>.h.
...
// reticulum-lora-repeater: the sibling project's NRF52840_XXAA branch
// redefines `SPI` to point at a custom NRF_SPIM2 `spiModem` instance
// created in its Config.h. We use the default Arduino global `SPI`
// instead, with our SPI.setPins() override inside preInit() driven
// by RADIO_SPI_OVERRIDE_PINS in the board header. TECHO path is
// preserved in case someone ports that board later.
...
bool sx126x::preInit() {
  ...
  // reticulum-lora-repeater: replaced the upstream per-BOARD_MODEL #if
  // chain with a macro-driven selector. The board header declares
  // RADIO_SPI_OVERRIDE_PINS = 1 if its PlatformIO board's default SPI
  // pins do not match its actual wiring (e.g. Faketec piggy-backs on
  // pca10056 whose default MOSI=45 collides with our CS). When the
  // override is set we must call SPI.setPins() BEFORE SPI.begin() or
  // the chip never sees a clock edge and this function times out
  // reading the sync word register.
...
void sx126x::enableTCXO() {
  // reticulum-lora-repeater: replaced the upstream per-BOARD_MODEL #if
  // chain with a macro-driven selector. The board header declares
  // RADIO_TCXO_VOLTAGE_MV (e.g. 1800 for Ebyte E22 modules, 3300 for
  // RAK4631), and we pick the SX1262 TCXO control value from that.
  // HAS_TCXO still gates the whole block — if the board header sets
  // HAS_TCXO 0 the driver never programs DIO3.
  #if HAS_TCXO
    #if !defined(RADIO_TCXO_VOLTAGE_MV)
      #error "RADIO_TCXO_VOLTAGE_MV must be defined in the board header when HAS_TCXO is true"
    #endif
    #if RADIO_TCXO_VOLTAGE_MV >= 2500
...
// reticulum-lora-repeater: the sibling project exports lora_low_datarate
// as a file-scope global in Config.h that several subsystems read.
// We don't have any other reader, so we keep it as a translation-unit-
// local static here to satisfy the driver without polluting the rest
// of the firmware. If a future subsystem needs it, promote to extern.
static bool lora_low_datarate = false;
